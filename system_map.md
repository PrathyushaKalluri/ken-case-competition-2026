...
**Confidence update, from directly fetching a primary NCH/PIB document** (`evidence/secondary_research/government_data/nch_pib_press_release_oct2025.txt`): the specific claim below — that household appliance servicing sits in the NCH's top-5 complaint categories, with the 6 named buckets — originally came from a WebSearch AI-synthesis, not a primary document. A real primary PIB press release (Oct 2025) was subsequently fetched and text-extracted directly; it verifies NCH's overall scale (1,70,585 average monthly complaints in 2025, up from 37,062 in 2017) and names **e-commerce**, not appliances, as the largest single category in its own July 2025 refund breakdown. **Treat the specific appliance-servicing ranking and the 6-bucket breakdown below as directionally plausible (multiple independent secondary sources converge on it) but not yet primary-source-verified** — downgraded from an unqualified [LAW] tag accordingly. This is exactly the kind of correction this project's own evidence discipline exists to catch — noted here rather than quietly left overstated.

Consumer Affairs Ministry data tabled in Parliament (2024), as reported via secondary sources, describes **"service of household appliances" as sitting in the top five complaint categories on the National Consumer Helpline, year after year** — alongside e-commerce, telecom, banking, and electricity. Complaints registered per month rose from 37,062 in 2017 to 1,12,468 in 2024 per that same secondary reporting (a similar but not identical figure to the primary document's own 1,70,585-by-2025 trajectory — the two are consistent with continued growth, not contradictory). Six recurring complaint buckets are named specifically in the secondary source:
